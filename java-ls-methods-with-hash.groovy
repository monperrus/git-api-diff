...
//
// outputs the list of functions of a Java file passed as parameter, used by git diff to find modified methods
...
class Lister extends AbstractProcessor<CtMethod> {
  ...
  void process(CtMethod c) {
    result.add(c.getParent().getQualifiedName()+"#"+c.getSignature()+" "+c.toString().hashCode());
  }
}
